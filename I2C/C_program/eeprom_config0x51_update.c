...
int main(int argc, char * argv[])
{
	...
	printf ("Testing Utility EEPROM:0x51\n");
/*
	...
	else if(a==2)
	{
		chip=0x51;
		dwordsMin=0x00;
		dwordsMax=0x100; //change according to utility eeprom
	}
	else
	{
...
*/
	...
	//char* i2cset = "i2cset -m 0xff -y %d %d %d %d"; //'bus' 'chip' 'addr' 'value'	
	char* i2cset = "i2cset -y %d %d %d %d %d  -i"; //'bus' 'chip' 'Haddr' 'Laddr'   'value'
	char* i2csetBeforeGet = "i2cset -y %d %d %d %d"; //'bus' 'chip' 'Haddr' 'Laddr' //set address counter
        char* i2cget = "i2cget -y %d %d ";	//'bus' 'chip' //read from the set address 
	...
	//need o re design the loops for read, write operations- with higher & lower byte addresses
	for (int j = 0; j<2; i++)
	...
	{
		//sprintf (cmd, i2csetBeforeGet, bus, chip, i,i2);
		//run_command(cmd, output);
		
		sprintf (cmd, i2cget, bus, chip, i);
		run_command(cmd, output);
		printf("\n%s \t->|%s",cmd,output);
		int num = (int)strtol(output, NULL, 16); 
	//	printf("\ti = %d , stroi = %d n",i,num);
